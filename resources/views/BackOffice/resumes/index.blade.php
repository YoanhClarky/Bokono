                ...
                <div class="col-lg-12">
                    <div class="container mt-3">
                        <div class="row justify-content-left" style="padding-bottom: 5px">
                                        <a class="btn btn-primary" href="/dashboard/resume/creation">NOUVEAU RESUME</a>
                            </div>
                        </div>
                    </div>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            <a href="/dashboard/resume/show/{{ $item->token }}" class="table-link">
                                                <i class="fas fa-book"></i> {{ " - ".$item->designation}}
                                            </a>
                                        </div>
                                        <div>
                                            @if($item->etat)
                                            <a class="btn btn-success btn-sm" href="/dashboard/resume/desactiver/{{ $item->token }}">
                                                <i class="fas fa-check"></i> Est actif
                                            </a>
                                            @else
                                            <a class="btn btn-warning btn-sm" href="/dashboard/resume/activer/{{ $item->token }}">
                                                <i class="fas fa-times"></i> Est inactif
                                            </a>
                                            @endif
                                            <a class="btn btn-danger btn-sm" href="/dashboard/resume/supprimer/{{ $item->token }}">
